...
use Google\Client;
use DDD\Domain\Google\GoogleCredentials;
...
class GoogleAuthService
{
    ...
    public function storeCredentials($code): GoogleCredentials | \Throwable
    {
        /**
        ...
        try {
            ...
            $googleCredentials = GoogleCredentials::firstOrCreate(
                [
                    'user_id' => auth()->id()
                ],
                [
    ...
    public function getGoogleUserCredentials(User $user): GoogleCredentials
    {
        $googleCredentials = GoogleCredentials::where('user_id', $user->id)->first();

        $this->client->setAccessToken($googleCredentials->token);

        if ($this->client->isAccessTokenExpired()) {
            $this->refreshAccessToken($googleCredentials);
        }

        return $googleCredentials;
    }

    private function refreshAccessToken(GoogleCredentials $googleCredentials)
    {
        ...
        $googleCredentials->token = $this->client->getAccessToken();
        $googleCredentials->save();
    }
}
